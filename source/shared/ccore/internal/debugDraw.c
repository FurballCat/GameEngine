...
void fc_dbg_init(fc_alloc_callbacks_t* pAllocCallbacks)
{
	...
	g_debugFragments.linesData = FUR_ALLOC(sizeof(float) * 2 * FC_DEBUG_VERTEX_SIZE * FC_DEBUG_FRAGMENTS_LINES_CAPACITY, 8, FC_MEMORY_SCOPE_DEBUG, pAllocCallbacks);
...
void fc_dbg_line(float start[3], float end[3], float color[4])
{
	...
	FUR_ASSERT(idx == g_debugFragments.numLines * 2 * FC_DEBUG_VERTEX_SIZE);
}
