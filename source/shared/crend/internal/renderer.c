...
const char* frInterpretVulkanResult(VkResult result)
{
	switch(result)
	{
			...
			FUR_CASE_ENUM_TO_CSTR(VK_ERROR_INCOMPATIBLE_DRIVER, " - probably missing Vulkan SDK");
					   ...
					   struct FrAllocationCallbacks*	pAllocCallbacks)
{
	...
	if(res == FR_RESULT_OK)
	{
		...
		pRenderer->bindingDescription[1].stride = sizeof(FrVector3);
